...
# general_dict['filter_list'] = np.concatenate((df_orig.filter_one.unique(),df_orig.filter_two.unique(),df_orig.filter_three.unique()))
# general_dict['full_filter_list'] = np.concatenate((df_orig.filter_one.unique(),df_orig.filter_two.unique(),df_orig.filter_three.unique()))
...
def update_filters():
    ...
    for type_filter in general_dict['type_list']:
        ...
        # print(':::',general_dict['individual_filters'][type_filter])

def update_filter(type_filter):
    common_filters = [general_dict['filter_3'].labels[i] for i in general_dict['filter_3'].active] + general_dict['filter_1'].value + general_dict['filter_4'].value
    uncommon_filters = general_dict['individual_filters_vis'][type_filter].value
    # print(':::',uncommon_filters)
    general_dict['individual_filters'][type_filter] = common_filters + uncommon_filters# + [type_filter]
...
def create_points_plot_layout(points_plot_name):
    ...
    # general_dict[points_plot_name] = figure(x_range=(0, 10), y_range=(0, 10), plot_width=500, plot_height=500, match_aspect=True, tools=['tap'])
    ...
    # general_dict['points_plot_names'].append(points_plot_name)
...
def calculate_points(type_filter):
    ...
    # print("calculate_points", timeit.timeit('output = 10*5'))
...
data_source = ColumnDataSource(pd.DataFrame(columns=['filter_one', 'filter_two', 'filter_three', 'filter_four']))
...
def init_plot_points(type_filter):
    ...
    # print("1: ", calculate_points_old(type_filter)['visual_sum'])
    # print("2: ",filtered_df_points['visual_sum'])
...
def update_plot_points(type_filter):
    ...
    # print(general_dict[type_filter].data)

general_dict['points_plots'] = []
for type_filter in general_dict['type_list']:
    general_dict['points_plots'].append(init_plot_points(type_filter))

# plots = gridplot([general_dict['points_plots']], sizing_mode='scale_both')
...
# row_one = row(general_dict['filter_3'], sizing_mode='scale_width', css_classes=['super-flex'], name='row_one')
...
buttons_row = row(column(children=[general_dict['filter_1'], button_1, button_2], height=200, max_height=200), column(children=[general_dict['filter_4'], button_3, button_4], height=200, max_height=200), data_table)
...
for type_filter in general_dict['type_list']:
    ...
    # general_dict['individual_filters_vis'][type_filter].on_change('value', callback_individual_filters) #partial(callback, foo="plus")
...
def init_emotions_points(type_filter):
    ...
    general_dict['emotions_rec_score'][type_filter] = ColumnDataSource(dict(right=[rec_score], left=[rec_score],))
    general_dict['emotions_easy_score'][type_filter] = ColumnDataSource(dict(right=[easy_score], left=[easy_score],))
...
def update_emotions_points(type_filter):
    ...
    if not rec_score or not easy_score:
        rec_score = 0.0
        easy_score = 0.0

    # Update ColumnDataSource
    general_dict['emotions_rec_score'][type_filter].data = dict(right=[rec_score], left=[rec_score],)
    general_dict['emotions_easy_score'][type_filter].data = dict(right=[easy_score], left=[easy_score],)
    # general_dict[type_filter].data = filtered_df_points
...
# plots_emotions = gridplot([general_dict['emotions_plots']], sizing_mode='scale_both')#, сss_classes=[])
...
def create_frequency_plot_layout(points_plot_name, y_range):
    ...
    general_dict[points_plot_name] = figure(y_range=y_range, plot_height=450, plot_width=500, match_aspect=True, toolbar_location=None, tools="", outline_line_color=None, output_backend="webgl", name='freq_f')
...
def init_frequency_points(type_filter):
    ...
    for lemmas_row in selected_lemmas_pv.itertuples():
        for word in lemmas_row[1]:
            if not word in d_pv:
                d_pv[word] = np.array([[lemmas_row[2],lemmas_row[3],lemmas_row[4]]])
            d_pv[word] = np.vstack((d_pv[word],np.array(  [[lemmas_row[2],lemmas_row[3],lemmas_row[4]]]  ) ))
    ...
    for lemmas_row in selected_lemmas_pv.itertuples():
        for word in lemmas_row[1]:
            if not word in d_uv:
                d_uv[word] = np.array([[lemmas_row[2],lemmas_row[3],lemmas_row[4]]])
            d_uv[word] = np.vstack((d_uv[word],np.array(  [[lemmas_row[2],lemmas_row[3],lemmas_row[4]]]  ) ))
    ...
    for lemmas_row in selected_lemmas_pv.itertuples():
        for word in lemmas_row[1]:
            if not word in d_nv:
                d_nv[word] = np.array([[lemmas_row[2],lemmas_row[3],lemmas_row[4]]])
            d_nv[word] = np.vstack((d_nv[word],np.array(  [[lemmas_row[2],lemmas_row[3],lemmas_row[4]]]  ) ))
    
    d_freq_pv = {k: sum([1 if item[0] in general_dict['individual_filters'][type_filter] and item[1] in general_dict['individual_filters'][type_filter] and item[2] in general_dict['individual_filters'][type_filter] else 0 for item in v[:]]) for k, v in d_pv.items()}
    d_freq_uv = {k: sum([1 if item[0] in general_dict['individual_filters'][type_filter] and item[1] in general_dict['individual_filters'][type_filter] and item[2] in general_dict['individual_filters'][type_filter] else 0 for item in v[:]]) for k, v in d_uv.items()}
    ...
    d_freq_nv = {k: sum([1 if item[0] in general_dict['individual_filters'][type_filter] and item[1] in general_dict['individual_filters'][type_filter] and item[2] in general_dict['individual_filters'][type_filter] else 0 for item in v[:]]) for k, v in d_nv.items()}
...
# plots_frequencies = gridplot([general_dict['frequencies_plots']], sizing_mode='scale_both')
...
# plots_wordclouds = gridplot([general_dict['wordcloud_plots']], sizing_mode='scale_both')
...
###################################################################################
###################################################################################
# row_list = [row_one,row_two,row_three,row_four,row_five,row_six,row_seven]
# for row in row_list:
#     row.align = 'start' 
    # curdoc().add_root(row)
# curdoc().add_root(row_one)

curdoc().add_root(column(column(row_one,buttons_row, row_three,row_four,row_five,row_six,row_seven, sizing_mode='scale_width', css_classes=['upper-flex']), sizing_mode='scale_width', margin=(0,30,0,0)))

# curdoc().add_root(column(column(row_one,row_two,row_three,row_four,row_five,row_six,row_seven, sizing_mode='scale_width', css_classes=['upper-flex']), sizing_mode='scale_width', margin=(0,30,0,0)))


#plots_wordclouds
# curdoc().add_root(column(general_dict['filter_3'], row(list(general_dict['individual_filters_vis'].values()), sizing_mode='scale_width'), plots, plots_emotions, plots_frequencies, plots_wordclouds, sizing_mode='scale_both'))
# curdoc().add_root(column(general_dict['filter_3'], general_dict['filter_1'], row(list(general_dict['individual_filters_vis'].values()), sizing_mode='scale_width'), plots, plots_emotions, plots_frequencies, plots_wordclouds, sizing_mode='scale_both', css_classes=['upper-flex'], margin=(0,25,0,0)))
